...
RUSTFLAGS="-C opt-level=0 -C no-prepopulate-passes -C passes=name-anon-globals -Cdebuginfo=2 -Cembed-bitcode=yes -Awarnings" cargo rustc --release --bin $1 --target-dir "." -- --emit=llvm-bc -Clto=fat
...
cp release/deps/*.bc original.bc
rm -rf release
...
$root_path/genExp.sh original.bc
