...
def getUnsafeLines(fname):
    ...
    for ln, line in enumerate(lines):
        if "get_unchecked(" in line or "get_unchecked_mut(" in line:
            line_nums.append(ln + 1)

    return line_nums


def genSourceExpNB(explore_abs, old_fname, new_fname, exp_num, line_nums):
    os.chdir(explore_abs)
    os.chdir("..") # go to cargo_root

    # convert and save to new file
    convertFile(old_fname, new_fname, line_nums)

    # compile to original.bc
    dir_name = os.path.join(explore_abs, "exp-" + str(exp_num))
    ...
    return subprocess.Popen(["../../../oneGenFromSource.sh"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def genAllFirstRoundExp(cargo_root, old_fname, new_fname, all_line_nums):
    ...
    for idx, line_num in enumerate(all_line_nums):
        ...
        child_processes.append(genSourceExpNB(explore_abs, old_fname, new_fname, idx, test_line_nums))
...
def secondRoundExp(cargo_root, old_fname, new_fname, impact_tuple, arg=None):
    sorted_line_nums = [x[1] for x in impact_tuple]
